...
flags.DEFINE_integer("hidden_units", 100,
                     "Number of units in the hidden layer of the NM")
...
flags.DEFINE_integer("replicas_to_aggregate",None,
                     "Number of replicas to aggrate before parameter"
...
flags.DEFINE_string("worker_host",
...
def main(unsaved_argv):
    ...
    with tf.device(
        ...
        if FLAGS.sync_replicas:
            if FLAGS.replicas_to_aggrate is None:
                replicas_to_aggrate = num_workers
            else:
                replicas_to_aggrate = FLAGS.replicas_to_aggrate

            opt = tf.train.SyncReplicasOptimizer(
                opt,
                replicas_to_aggregate = replicas_to_aggrate,
        ...
        print("tTraining begins @ %f" % time_begin)

        local_step = 0
        while True:
            batch_xs,batch_ye = mnist.train.next_batch(FLAGS.batch_size)
            train_feed = {x:batch_xs,y_:batch_ye}
